...
class JuliaProject(CondaProject, JuliaProjectTomlBuildPack):
    ...
    def __init__(self, project_path, env_base_path, log, dry_run=False, **kwargs):
        ...
        if self.detected and Version(self.interpreter_version) >= Version(self.JULIA_SUPPORTS_NAMED_ENV):
    ...
    def create_environment(self, **kwargs):
        ...
        cmds = [
            ["juliaup", "add", v],
            ["julia", f"+{v}", f"--project='{self.julia_project_dir}'", "-e", "using Pkg; Pkg.add(\"IJulia\"); Pkg.resolve(); Pkg.instantiate();"]
        ]
    ...
    def create_kernel(self, name="", display_name = "", user=False, prefix="", **kwargs):
        ...
        cmds = [
            ["julia", f"+{self.interpreter_version}", f"--project='{self.julia_project_dir}'", "-e", f"using Pkg; using IJulia; installkernel(\"{_name}\", \"--project='{self.julia_project_dir}'\", displayname=\"{_display_name}\", env=Dict(\"JULIA_DEPOT_PATH\"=>\"{env.get('JULIA_DEPOT_PATH')}\"));"],
        ]
